...
# Library exceptions
# *******************
class YaMongoException(Exception):
    """
...
class YaMongoInitError(YaMongoException):
    """
...
class YaMongoCollectionError(YaMongoException):
    """
...
def create_collection_class(class_name: str, collection_name: Union[str, None] = None):
    ...
    if __db is None:
        raise YaMongoInitError('init_db function must be called before creation of collection classes')
    collection_class = type(class_name, (Document, ), {
        'collection': __db[collection_name]
    })
